...
| B5 — Documentação e vídeo | [Parecer](../reports/reviews/rodada-1/B5.md) | [Quarto parecer](../reports/reviews/B5.md) | Oito cenas, monitoração demonstrada, vínculo do gerador, versões, ambientes e narração |

Os manifestos acompanham os pareceres com sufixo `-manifest.json`. Há **17 pareceres concluídos**: quatro de B1, dois de B2, três de B3, quatro de B4 e quatro de B5. Os diretórios `rodada-1` a `rodada-5` preservam snapshots anteriores, incluindo tentativas que falharam. Nessas chamadas concluídas, a solicitação foi `fable` e a CLI reportou `claude-fable-5-1`. Também listou `claude-haiku-4-5-20251001` entre os modelos utilizados internamente; o registro original foi preservado. O script não substituiu o modelo solicitado nem atribui artificialmente cada trecho do parecer a um modelo.

As duas tentativas suplementares B1 que receberam HTTP 429 estão preservadas em `rodada-3/B1.*` e `rodada-4/B1.*`; elas não contam como parecer. Após a renovação do limite, duas novas revisões B1 foram concluídas, em `rodada-5/B1.*` e `reports/reviews/B1.*`. A revisão B5 de oito cenas também terminou. As correções posteriores aos snapshots são verificadas pelo orquestrador e identificadas abaixo, sem convertê-las em aprovação automática do Claude.
...
A versão anterior do vídeo tinha 219,626395 segundos, H.264/AAC, 1280×720 e 451 palavras, com API real do host em 8004. Seus sete quadros foram extraídos e inspecionados, e o verificador então passou em 26 casos. Nessa versão histórica, o CI aparecia como configuração estática. A auditoria posterior acrescentou a explicação de monitoração e o CI real identificado, conforme a seção seguinte; os artefatos atuais em `reports/video/` substituem essa captura.
...
Na terceira rodada, o verificador ampliado passou em **26 casos**, incluindo preservação de intermediários, e o MP4 foi renderizado novamente. A [rejeição de versão](../reports/video/rejeicao_versao.json) preserva a evidência daquele gerador. O CI remoto 34905435615 concluiu com sucesso depois do snapshot da revisão. A inspeção e a verificação atuais em `reports/video/` correspondem à versão posterior de oito cenas.
...
O quarto parecer B5 revisou o gerador de oito cenas e a explicação de monitoração. Foram corrigidos o vínculo do hash do gerador, a comparação da narração de cada cena, a exigência de DAG real no verificador final, o rodapé dinâmico, a porta padrão 8004, o redirecionamento do processo PowerShell e a comparação independente das fontes. Os comandos de reprodução agora usam `smoke_stack_novo.json`, preservando as capturas históricas. O [registro dos comandos](../reports/docker/pos_correcao/benchmark_http_execucao.json) comprova a captura posterior do smoke da cena 6; contagem acumulada e janela do histograma são explicadas no roteiro. O commit local `fa821ef` e o remoto `3a7ad7f` têm a mesma árvore Git, conforme a matriz.

O vídeo posterior à revisão tem **260,876417 segundos**, oito cenas e 525 palavras de narração sintética. O [verificador completo](../reports/video/verificacao.json) passou em **67 verificações**, incluindo preservação do MP4, manifesto e 26 intermediários, além do vínculo das fontes e narração. O [roteiro](roteiro_star.md) identifica os experimentos históricos, a stack Docker e o commit real do CI mostrado. O parecer estático precede essas correções; a execução do verificador e a inspeção do MP4 são evidências do orquestrador.
...
As correções da auditoria passaram no [CI final 34921095963](../reports/ci/34921095963/execucao.json), commit `6426446`, com **104 testes aprovados**, builds, quatro tarefas Airflow e stack. O [runtime local atualizado](../reports/auditoria_criterios/runtime_final.json) executa as mesmas fontes de treinamento/pipeline, conferidas por SHA-256, e reutiliza o modelo preservado. Os registros finais de CI/runtime e sua consolidação documental são posteriores ao commit testado; não alteram o código validado.
