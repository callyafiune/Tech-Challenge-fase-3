...
A tentativa adicional de diagnóstico B1 recebeu limite de sessão HTTP 429, preservado em `reports/reviews/B1.*`; ela não conta como parecer. O último parecer válido B1 está em `rodada-2`. A terceira B5 terminou antes desse limite. As correções posteriores ao seu snapshot foram verificadas localmente e são descritas abaixo, sem alegar uma nova rodada inexistente.
...
O [estado histórico da construção](../reports/airflow_build_state.json) registra uma tentativa interrompida por falta de espaço. A [construção posterior](../reports/docker/build_airflow_state.json) terminou com código zero em 124,2 segundos. A [execução real](../reports/airflow_execucao.json) concluiu as quatro tarefas, publicou uma nova versão e confirmou a saúde de metadatabase, scheduler e triggerer do standalone. O comando `dags test` levou 66,6 segundos, conforme o registro de comandos; a data lógica da DAG não mede esse intervalo. A execução remota inicial falhou no gate de paridade ONNX. A ocorrência não se repetiu na DAG diagnóstica posterior; foi adicionada instrumentação de erro e os critérios permaneceram iguais. Nela, o Airflow 2.11 aplicou `retries=1` também no comando `dags test`; a documentação anterior que restringia esse comportamento ao agendador foi corrigida.
...
A última suíte em [testes.xml](../reports/testes.xml) registra 76 casos aprovados e um ignorado, sem falhas ou erros. O teste ignorado depende do Airflow real; o sucesso da DAG foi verificado separadamente no container. A suíte local não comprova execução remota do GitHub.
