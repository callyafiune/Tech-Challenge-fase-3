...
| CI/CD — GitHub Actions | 15% | Workflow por push com lint, testes, builds, importação e execução da DAG e verificação da stack. A imagem disponibilizada para entrega é a mesma validada pelo CI. [Workflow](../.github/workflows/ci.yml). | Aguardando a execução final das correções |
