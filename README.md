...
O [workflow de entrega](.github/workflows/entrega.yml) publica manualmente no GHCR a partir de `main`, depois do CI. Ele baixa a imagem construída e verificada pelo CI, confere SHA-256 e ID, carrega o arquivo e publica **sem reconstrução**, com tag do commit e `latest`. A [execução remota inicial](https://github.com/callyafiune/Tech-Challenge-fase-3/actions/runs/34904519738) falhou no gate de paridade ONNX durante o treinamento da DAG. A ocorrência não se repetiu no CI posterior: foi adicionada instrumentação de erro e os mesmos critérios foram preservados, sem declarar uma causa corrigida. **A publicação GHCR ainda não foi comprovada**. A proposta AWS/ECR também não foi provisionada.
